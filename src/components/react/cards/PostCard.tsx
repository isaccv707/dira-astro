...
const PostCard = ({ post }: PostCardProps) => {
    ...
    return (
        <article
            className="w-full h-full bg-white border border-gray-100 rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col overflow-hidden group"
        >
                ...
                image && (
                    <div className="relative aspect-[16/9] overflow-hidden">
                        ...
                        <div className="absolute top-4 left-4">
                             <span className="px-3 py-1 bg-white/90 backdrop-blur-sm text-green-ligth text-[10px] uppercase tracking-widest font-bold rounded-full shadow-sm">
                                Blog
                             </span>
                        </div>
                    </div>
            ...
            <div className="p-6 md:p-8 flex flex-col flex-1">
                ...
                <div className="flex-1 flex flex-col gap-4">
                        ...
                        tags && tags.length > 0 && (
                            <div className="flex flex-wrap gap-2">
                                {tags.map((tag, index) => (
                                    <span 
                                        key={index}
                                        className="px-2.5 py-1 bg-green-ligth/5 text-green-ligth text-[10px] uppercase tracking-wider font-bold rounded-lg border border-green-ligth/10"
                                    >
                                        #{tag}
                                    </span>
                                ))}
                    ...
                    <h2 className="text-xl md:text-2xl font-bold text-green-ligth leading-tight line-clamp-2 group-hover:text-green-primary transition-colors duration-300">
                        {title}
                    </h2>
